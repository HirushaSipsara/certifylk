...
def upgrade() -> None:
    ...
    conn.execute(
        sa.text("""
            ...
            VALUES (:id, 'Food Products', 'food_products',
                    'Manufactured food and beverage products for sale in Sri Lanka.', 1, 1)
    ...
    conn.execute(
        sa.text("""
            ...
            ON CONFLICT (slug) DO NOTHING
        """),
        {"id": PROD_CORDIAL, "cat_id": CAT_FOOD_PRODUCTS},
    )
        ...
        sa.text("""
            ...
            VALUES
              ('SLS_MARK_CORDIAL', 'SLS Mark — Fresh Fruit Cordial', 'SLS_MARK',
               ...
               365, 'https://www.slsi.lk/product-certification.html', 1, 1)
            ON CONFLICT (id) DO NOTHING
        """),
        {
        ...
        sa.text("""
            ...
            VALUES
              ('ISO_22000', 'ISO 22000 Food Safety Management', 'ISO_22000',
               ...
               540, 'https://www.iso.org/iso-22000-food-safety-management.html', 0, 1)
            ON CONFLICT (id) DO NOTHING
        """),
    ...
    def req(rid: str, cat: str, title: str, desc: str, weight: float, safety: bool,
            doc: str, clause: str, rule: str, order: int) -> None:
        conn.execute(
            sa.text("""
                ...
                VALUES
                  (:id, 'SLS_MARK_CORDIAL', :cat, :title, :desc, :weight,
                   :safety, :doc, :clause, '', 0, :rule, :order, 1)
                ON CONFLICT (id) DO NOTHING
            """),
            {
                "id": rid, "cat": cat, "title": title, "desc": desc,
                "weight": weight, "safety": 1 if safety else 0,
                "doc": doc, "clause": clause, "rule": rule, "order": order,
            },
        )

    # Hygiene & Sanitation (weight total: 20)
    req("SLS_HYG_HANDWASH", "Hygiene & Sanitation",
        ...
        6, True,
        "SLS 187 / GMP Guidelines (SLSI)", "Clause 4.2.1 — Personal Hygiene",
        '{"question": "HYG_HAND_01", "confirmed": ["always"], "partial": ["sometimes"], "gap": ["never"]}', 1)

    req("SLS_HYG_CLEANING", "Hygiene & Sanitation",
        ...
        6, True,
        "SLS 187 / GMP Guidelines (SLSI)", "Clause 4.3.1 — Cleaning Programme",
        '{"question": "HYG_CLEAN_01", "confirmed": ["recorded_each_batch"], "partial": ["routine_no_record"], "gap": ["only_when_dirty"]}', 2)

    req("SLS_HYG_CHEMICAL", "Hygiene & Sanitation",
        ...
        4, True,
        "SLS 187 / GMP Guidelines (SLSI)", "Clause 4.3.3 — Chemical Control",
        '{"question": "HYG_CHEM_01", "confirmed": ["locked_separate"], "partial": ["separate_area"], "gap": ["with_ingredients"]}', 3)

    req("SLS_HYG_PEST", "Hygiene & Sanitation",
        ...
        4, False,
        "SLS 187 / GMP Guidelines (SLSI)", "Clause 4.4 — Pest Control",
        '{"question": "HYG_PEST_01", "confirmed": ["logged"], "partial": ["checked_not_logged"], "gap": ["not_checked"]}', 4)

    # Process Control (weight total: 20)
    req("SLS_PROC_STAGES", "Process Control",
        ...
        5, False,
        "SLS 187 / GMP Guidelines (SLSI)", "Clause 5.1 — Process Definition",
        '{"derived": "process_steps"}', 5)

    req("SLS_PROC_TEMP", "Process Control",
        ...
        7, True,
        "SLS 187", "Clause 5.3.2 — Thermal Process Control",
        '{"question": "PROC_TEMP_01", "confirmed": ["thermometer"], "partial": ["time_and_appearance"], "gap": ["appearance_only"]}', 6)

    req("SLS_PROC_FILL", "Process Control",
        ...
        4, True,
        "SLS 187", "Clause 5.4 — Filling and Closing",
        '{"question": "PACK_FILL_01", "confirmed": ["dedicated_clean_area"], "partial": ["cleaned_shared_area"], "gap": ["uncontrolled_area"]}', 7)

    req("SLS_PROC_SEP", "Process Control",
        ...
        4, True,
        "SLS 187 / GMP Guidelines (SLSI)", "Clause 5.2 — Cross-contamination Prevention",
        '{"question": "PROC_SEP_01", "confirmed": ["separate_time_area"], "partial": ["clean_between"], "gap": ["same_without_cleaning"]}', 8)

    # Documentation & Records (weight total: 15)
    req("SLS_DOC_BATCH", "Documentation & Records",
        ...
        6, False,
        "SLS 187", "Clause 7.1 — Batch Records",
        '{"question": "DOC_BATCH_01", "confirmed": ["always"], "partial": ["sometimes"], "gap": ["never"]}', 9)

    req("SLS_DOC_CLEANING", "Documentation & Records",
        "Cleaning records",
        "Cleaning activities must be recorded with date, area/equipment cleaned, cleaning agent used, and responsible person.",
        5, False,
        "SLS 187 / GMP Guidelines (SLSI)", "Clause 7.2 — Cleaning Records",
        '{"question": "DOC_CLEAN_01", "confirmed": ["every_day"], "partial": ["sometimes"], "gap": ["never"]}', 10)

    req("SLS_DOC_COMPLAINT", "Documentation & Records",
        ...
        4, False,
        "SLS 187", "Clause 7.4 — Complaint Handling",
        '{"question": "DOC_COMPLAINT_01", "confirmed": ["always"], "partial": ["sometimes"], "gap": ["never"]}', 11)

    # Raw Material & Supplier Control (weight total: 15)
    req("SLS_SUP_SOURCE", "Raw Material & Supplier Control",
        ...
        6, True,
        "SLS 187", "Clause 6.1 — Raw Material Sourcing",
        '{"question": "SUP_SOURCE_01", "confirmed": ["approved_regular"], "partial": ["known_variable"], "gap": ["unknown_cash"]}', 12)

    req("SLS_SUP_REGISTER", "Raw Material & Supplier Control",
        "Supplier register",
        "A register of all suppliers including name, contact, and ingredients supplied must be maintained and kept current.",
        5, False,
        "SLS 187 / GMP Guidelines (SLSI)", "Clause 6.2 — Supplier Register",
        '{"question": "SUP_REG_01", "confirmed": ["complete"], "partial": ["informal"], "gap": ["none"]}', 13)

    req("SLS_SUP_INCOMING", "Raw Material & Supplier Control",
        ...
        4, False,
        "SLS 187", "Clause 6.3 — Incoming Inspection",
        '{"question": "SUP_CHECK_01", "confirmed": ["check_record"], "partial": ["check_no_record"], "gap": ["no_check"]}', 14)

    # Packaging & Labelling (weight total: 15)
    req("SLS_PACK_LABEL", "Packaging & Labelling",
        ...
        7, False,
        "Food Act No. 26 of 1980 / SLS 187", "Clause 9 — Labelling Requirements",
        '{"question": "PACK_LABEL_01", "confirmed": ["complete"], "partial": ["some_details"], "gap": ["name_only"]}', 15)

    req("SLS_PACK_FOODGRADE", "Packaging & Labelling",
        ...
        5, False,
        "SLS 187", "Clause 8.1 — Packaging Materials",
        '{"question": "PACK_GRADE_01", "confirmed": ["documented"], "partial": ["supplier_statement"], "gap": ["none"]}', 16)

    req("SLS_PACK_SHELF", "Packaging & Labelling",
        ...
        3, False,
        "Food Act No. 26 of 1980 / SLS 187", "Clause 9.4 — Date Marking",
        '{"question": "PACK_DATE_01", "confirmed": ["supported"], "partial": ["experience"], "gap": ["none"]}', 17)

    # Storage & Traceability (weight total: 15)
    req("SLS_STORE_INGREDIENT", "Storage & Traceability",
        ...
        4, False,
        "GMP Guidelines (SLSI)", "Clause 4.5 — Ingredient Storage",
        '{"question": "STORE_RAISED_01", "confirmed": ["raised_closed"], "partial": ["raised_open"], "gap": ["on_floor"]}', 18)

    req("SLS_STORE_FINISHED", "Storage & Traceability",
        ...
        3, False,
        "SLS 187", "Clause 5.5 — Finished Product Storage",
        '{"question": "STORE_FIN_01", "confirmed": ["separate_protected"], "partial": ["shared_protected"], "gap": ["unprotected"]}', 19)

    req("SLS_TRACE_BATCH", "Storage & Traceability",
        ...
        5, False,
        "SLS 187", "Clause 7.1.4 — Batch Identification",
        '{"question": "TRACE_CODE_01", "confirmed": ["every_batch"], "partial": ["date_only"], "gap": ["none"]}', 20)

    req("SLS_TRACE_DIST", "Storage & Traceability",
        ...
        3, False,
        "SLS 187", "Clause 7.3 — Distribution Records",
        '{"question": "TRACE_SALES_01", "confirmed": ["batch_customer"], "partial": ["sales_only"], "gap": ["none"]}', 21)

    # ── SLS Mark cost items ───────────────────────────────────────────────────
    def cost(cid: str, aref: str, title: str, ctype: str,
             ot_min: int, ot_max: int, rec_min: int, rec_max: int, note: str) -> None:
        conn.execute(
            sa.text("""
                ...
                ON CONFLICT (id) DO NOTHING
            """),
            {"id": cid, "aref": aref, "title": title, "ctype": ctype,
             "ot_min": ot_min, "ot_max": ot_max, "rec_min": rec_min, "rec_max": rec_max,
             "note": note, "eff": EFFECTIVE, "rev": REVIEWED},
        )

    cost("COST_SLS_APP_FEE", "SLS_APPLICATION",
         "SLSI Application and Certification Fee",
         "certifying_body_fee",
         50000, 150000, 25000, 75000,
         "SLSI fee schedule (estimate, 2024). Varies by production volume and product complexity. Contact SLSI for exact quote.")

    cost("COST_LAB_INITIAL", "SLS_LAB_TESTING",
    ...
    cost("COST_LABEL_REDESIGN", "SLS_LABEL_REDESIGN",
    ...
    cost("COST_SLSI_AUDIT", "SLS_AUDIT",
...
def downgrade() -> None:
    conn = op.get_bind()
    conn.execute(sa.text("DELETE FROM scheme_cost_items WHERE scheme_id LIKE 'SLS_%' OR scheme_id LIKE 'CAA_%' OR scheme_id LIKE 'ISO_%'"))
    conn.execute(sa.text("DELETE FROM scheme_requirements WHERE scheme_id LIKE 'SLS_%' OR scheme_id LIKE 'CAA_%'"))
    conn.execute(sa.text("DELETE FROM certification_schemes WHERE id IN ('SLS_MARK_CORDIAL', 'CAA_FOOD_REG', 'ISO_22000')"))
