...
- Evidence analysis reliability and visibility: certificate evidence is sent in bounded concurrent batches with database-sourced requirement context, exact pair coverage validation, and aggregated truthful fallback metadata. The browser now pauses on a provider/polarity/confidence review state and supports a real retry or evidence replacement before clarifications.
